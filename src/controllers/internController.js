...
const createIntern = async function(req,res){ 
 try{
        ...
        if(!validator.isValidFullName(name)){ return res.status(400).send({status: false, msg: "Name not valid"})}
        ...
        if(!validator.isValidMobile(mobile)){ return res.status(400).send({status: false, msg: "number is not valid"})}
