...
try{
    ...
    if(!validator.validlogolink(logoLink)){ return res.status(400).send({status: false, msg: "logo link is not valid"})}
...
try {
    ...
    let college = await collegeModel.findOne({name:collegeName,isDeleted:false}).select({_id:1,name:1,fullName:1,logoLink:1}).lean()
